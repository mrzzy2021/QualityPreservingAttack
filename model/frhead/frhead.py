...
from model.frhead.modules import *
from model.frhead.lib import ST_RenovateNet

from model.frhead.graph import HDM05Graph, STYLEORIGraph
...
class FRHEAD(nn.Module):
    ...
    def __init__(self,
                 # Base Params
                 num_class=60, num_point=25, num_frame=64, num_person=2, dataset=None, graph_args=dict(), in_channels=3,
        ...
        if dataset == 'hdm05':
            self.graph = HDM05Graph(**graph_args)
        elif dataset == '100style_ori':
            self.graph = STYLEORIGraph(**graph_args)
        else:
...
if __name__ == '__main__':
    model = FRHEAD(num_class=65, num_point=25, num_frame=60, num_person=1, dataset='hdm05',
                   graph_args={'labeling_mode': 'spatial'}, in_channels=3, base_channel=64, drop_out=0, adaptive=True,
                   # Module Params
    ...
    x = torch.randn(2, 3, 60, 25, 1)  # hdm05
    # model = FRHEAD(num_class=100, num_point=23, num_frame=60, num_person=1, dataset='100style_ori',
    #                graph_args={'labeling_mode': 'spatial'}, in_channels=3, base_channel=64, drop_out=0, adaptive=True,
    #                # Module Params
    #                cl_mode='ST-Multi-Level', multi_cl_weights=[0.1, 0.2, 0.5, 1], cl_version='V0',
    #                pred_threshold=0, use_p_map=True, )
    # # input shape should be (N, C, T, V, M)
    # x = torch.randn(1, 3, 60, 23, 1)  # 100style_ori
    label = torch.tensor([[10],[25]]).squeeze(1)
    output, cl_loss = model(x, label=label, get_cl_loss=True)
    print(output.shape)