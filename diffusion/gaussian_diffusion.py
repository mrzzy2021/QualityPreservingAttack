...
class GaussianDiffusion:
    ...
    def p_mean_variance(
        self, model, x, t, x0=None, clip_denoised=True, denoised_fn=None, model_kwargs=None
    ):
        ...
        if self.model_mean_type in [ModelMeanType.START_X, ModelMeanType.EPSILON]:  # THIS IS US!
            if self.model_mean_type == ModelMeanType.START_X:
                pred_xstart = process_xstart(model_output)
                w = _extract_into_tensor(self.sqrt_alphas_cumprod, t-1, x.shape)
            else:
                pred_xstart = process_xstart(
                    self._predict_xstart_from_eps(x_t=x, t=t, eps=model_output)
                )
                w = _extract_into_tensor(self.sqrt_alphas_cumprod, t - 1, x.shape)
            model_mean = (w*x0 - self.q_posterior_mean_variance(
    ...
    def our_attack_100style(
        ...
        B, D, _, motion_length = motion.shape
        src_kwargs = copy.deepcopy(model_kwargs)
        adv_kwargs = copy.deepcopy(model_kwargs)
        labels = src_kwargs['y']['action'].to(device)
        action_text = model_kwargs['y']['action_text'][0]
        all_classes = list(range(100))
        found_flag = False

        while not found_flag:
            all_classes.remove(adv_kwargs['y']['action'].item())
            adv_x0 = copy.deepcopy(motion)
            src_x0 = motion

            random_class = random.choice(all_classes)
            adv_kwargs['y']['action'] = torch.tensor([[random_class]])

            iter_num_list = list(range(max_iter_num))
            max_iter_bar = tqdm(iter_num_list, desc=f'perturbing {action_text}')
            adv_x0.requires_grad = True

            for iter_num in max_iter_bar:
                with torch.no_grad():
                    noise = torch.randn_like(adv_x0)
                    src_timestep = random.randint(timestep_range[0], timestep_range[1])
                    src_t = torch.full((adv_x0.shape[0],), src_timestep).to(device=adv_x0.device)
                    adv_xt = self.q_sample(adv_x0, src_t, noise=noise)
                    src_xt = self.q_sample(src_x0, src_t, noise=noise)
                    adv_mu = self.p_mean_variance(model, adv_xt, src_t, adv_x0, clip_denoised=False,
                                                           denoised_fn=None, model_kwargs=adv_kwargs)
                    src_mu = self.p_mean_variance(model, src_xt, src_t, src_x0, clip_denoised=False, denoised_fn=None,
                                                   model_kwargs=src_kwargs)
                    grad = adv_mu - src_mu

                    grad = torch.nan_to_num(grad)
                    target = (adv_x0 - grad).detach()

                loss = 0.5 * F.mse_loss(adv_x0, target, reduction="mean")
                loss.backward()
                adv_x0.grad.data /= adv_x0.grad.norm(
                    p=2, dim=[1,2,3], keepdim=True
                )
                actual_grad = adv_x0.grad.detach()
                adv_x0.data = adv_x0 - actual_grad
                adv_x0.grad.zero_()

                with torch.no_grad():
                    victim_model.eval()
                    if victim_type == 'style':
                        output_score = victim_model(adv_x0.detach().permute(0, 3, 1, 2).reshape(1, 60, -1))
                    elif victim_type == 'sttformer':
                        output_score = victim_model(adv_x0.detach().unsqueeze(-1).permute(0, 2, 3, 1, 4))
                    elif victim_type == 'skateformer':
                        index_t = torch.arange(adv_x0.size(-1)).expand(adv_x0.size(0), -1).to(device)
                        output_score = victim_model(adv_x0.detach().unsqueeze(-1).permute(0, 2, 3, 1, 4), index_t)
                    elif victim_type == 'frhead':
                        output_score, _ = victim_model(adv_x0.detach().unsqueeze(-1).permute(0, 2, 3, 1, 4),
                                                       label=labels.squeeze(1), get_cl_loss=True)
                    _, predicted = torch.max(output_score, 1)
                    if (predicted == labels.squeeze(1)).sum().item() == 0:
                        adv = adv_x0.detach().cpu().numpy()
                        ori = src_x0.detach().cpu().numpy()
                        os.makedirs(save_path, exist_ok=True)
                        np.save(os.path.join(save_path, f'{action_text}.npy'),
                                {'motion': ori, 'adv_samples': adv})
                        found_flag = True
                        break

    def our_attack_hdm05(
        self,
        motion,
        model,
        max_iter_num=30000,
        ...
        while not found_flag:
            ...
            for iter_num in max_iter_bar:
                with torch.no_grad():
                    ...
                    adv_xt = self.q_sample(adv_x0, src_t, noise=noise)
                    src_xt = self.q_sample(src_x0, src_t, noise=noise)
                    adv_mu = self.p_mean_variance(model, adv_xt, src_t, clip_denoised=False,
                                                           denoised_fn=None, model_kwargs=adv_kwargs)
                    src_mu = self.p_mean_variance(model, src_xt, src_t, clip_denoised=False, denoised_fn=None,
                                                   model_kwargs=src_kwargs)
                    grad = adv_mu - src_mu
